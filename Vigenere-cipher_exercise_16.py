...
# суть моей реализации в том что мы получаем ключ текста и ключ ключа - суммируем их и по индексу после ищем букву в алфавите из-за суммы индекса тем самым шифруя Вижинера
...
while True:
    ...
    try:
        ...
        for i in key:
            # проверка на то что каждый символ это буква так как шифр Вижинера шифрует с помощью ключа-слова
            if not i.isalpha():
                raise ValueError("Please enter only letters for KEY:")
